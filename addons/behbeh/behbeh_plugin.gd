...
static func dprint(s: String):
	BehTreeEditor.dprint(s) # See BehTreeEditor implementation for debug switch.


func _enter_tree():
	dprint("[BehBehPlugin] Entering tree.")
...
func _exit_tree():
	dprint("[BehBehPlugin] Exiting tree.")
...
#		docked_beh_editor.grab_focus()
	...
	dprint("[BehBehPlugin] _handles() returning false.")
	return false


func _make_visible(visible: bool):
	dprint("[BehBehPlugin] make_visible %s called." % visible)
	if visible:
...
func _edit(obj: Object):
	if obj is BehTree: dprint("[BehBehPlugin] _edit(): Got object to edit: BehTree")
	if obj is BehNode: dprint("[BehBehPlugin] _edit(): Got object to edit: BehNode")
	if obj == null: dprint("[BehBehPlugin] _edit(): Got object to edit: null")
	var to_edit = obj
	...
	if docked_beh_editor.active_tree != null && !is_first_edit_focus:
		dprint("[BehBehPlugin] _edit: taking focus b/c active_tree != null")
		make_bottom_panel_item_visible(docked_beh_editor)
	if is_first_edit_focus:
		...
		dprint("[BehBehPlugin] _edit: HACK: Skipping first edit focus...")
